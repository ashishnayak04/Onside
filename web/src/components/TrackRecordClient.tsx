      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
          <h1 className="page-header mb-1">
            Track Record<span className="text-ember">.</span>
          </h1>
        ...
        <div className="table-scroll">
          <table className="w-full text-sm" style={{ minWidth: "560px" }}>
            ...
            <tbody>
              {filtered.map((r, i) => (
                ...
                >
                  <td className="px-4 py-3">
                    <p className="font-semibold text-sm text-chalk">
                  ...
                  <td className="px-4 py-3 font-mono text-sm font-bold text-chalk">
                    {r.predicted_home_score as number} - {r.predicted_away_score as number}
                  </td>
                  <td className="px-4 py-3 font-mono text-sm text-stale">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {r.actual_outcome ? (
                      <span className="text-xs block text-stale" style={{ opacity: 0.7 }}>
                  ...
                  <td className="px-4 py-3">
                    {r.was_correct === true ? (
                      ...
                      <span
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold"
                        style={{
                          background: "rgba(217,59,62,0.10)",
                          color: "#B3272A",
                          fontFamily: "var(--font-label)",
                        }}
                      >
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-sm text-stale">
                    No results{activeFilter !== "All" ? ` for ${activeFilter}` : ""}.{" "}
                    {activeFilter !== "All" && (
                      <button
                        onClick={() => setActiveFilter("All")}
                        className="underline text-flame"
                      >
                        Show all
                      </button>
                    )}
